...
struct AuthService {
    ...
    func verifyOTP(phone: String, code: String) async throws -> AuthResponseDTO {
        ...
        guard let storedCode = try await req.redis.get(key, as: String.self), storedCode == code else {
            throw Abort(.unauthorized, reason: "Invalid OTP")
        }

        _ = try await req.redis.delete(key)
    ...
    func refresh() async throws -> AuthResponseDTO {
        ...
        guard try await req.redis.get(refreshKey, as: String.self) != nil else {
            throw Abort(.unauthorized, reason: "Refresh token revoked")
        }

        _ = try await req.redis.delete(refreshKey)
